function [orCalc,diamCalc,interCalc,endCalc] = parLooper(im,voxelSize,preOrCalc,preDiamCalc)
    ...
    CC = bwconncomp(interCalc);
    L = labelmatrix(CC);
    interCalcFix=interCalc;
    for i = 1:max(L(:))
        tmp = imdilate(L==i,strel('cube',3));
        tmp = tmp - logical(L==i);
        tmp2 = tmp .* (im>0-interCalc);
        CC = bwconncomp(tmp2);
        if CC.NumObjects<2
            interCalcFix(L==i)=0;
        end
    end
    interCalc = interCalcFix;
    
    
    
    edgeDistI=zeros(size(im));
    edgeDistJ=zeros(size(im));
    edgeDistK=zeros(size(im));
    for i = 1:size(im,1)
        edgeDistI(i,:,:) = voxelSize(2)*min([i-1, size(im,1)-i]);
    end
    for j = 1:size(im,2)
        edgeDistJ(:,j,:) = voxelSize(1)*min([j-1, size(im,2)-j]);
    end
    for k = 1:size(im,3)
        edgeDistK(:,:,k) = voxelSize(3)*min([k-1, size(im,3)-k]);
    end
    edgeDist = min(cat(4,edgeDistI,edgeDistJ,edgeDistK),[],4);
    fibsOnEdge = diamCalc>edgeDist;
    fibsOnEdgeDil = imdilate(fibsOnEdge,strel('cube',3));
    fibsOnEdge = fibsOnEdgeDil .* im>0;
    
    orCalc(repmat(fibsOnEdge,1,1,1,3)) = nan;
    orCalc(repmat(interCalc,1,1,1,3)) = nan;
    orCalc(repmat(endCalc,1,1,1,3)) = nan;
    diamCalc(fibsOnEdge) = nan;
    diamCalc(interCalc) = nan;
    diamCalc(endCalc) = nan;
